...
public class Finder {
	...
	public List<User> findUsers(String accountNumber, String accountName,
		...
		TypedQuery<User> query = entitymanager.createQuery(queryString,
				User.class);
	...
	public List<Project> findProjects(String projectNumber, String title,
			String type, String pincode) {
		createEntityManager();
		...
		if (StringUtils.isNotBlank(type) && !("none".equalsIgnoreCase(type))) {
			constraints.add("p.projectType='"
					+ ProjectType.valueOf(type).getValue() + "'");
		}
		...
		TypedQuery<Project> query = entitymanager.createQuery(queryString,
				Project.class);
	...
	public List<Document> findDocuments(String documentNumber, DocumentType type) {
		...
		TypedQuery<Document> query = entitymanager.createQuery(queryString,
				Document.class);
	...
	public List<Bill> findBills(String billNumber, BillType type) {
		...
		TypedQuery<Bill> query = entitymanager.createQuery(queryString,
				Bill.class);
	...
	public List<Payment> findPayments(String paymentNumber) {
		...
		TypedQuery<Payment> query = entitymanager.createQuery(queryString,
				Payment.class);
	...
	public List<Drawing> findDrawings(String drawingNumber) {
		...
		TypedQuery<Drawing> query = entitymanager.createQuery(queryString,
				Drawing.class);
	...
	public List<Document> findDocumentsByID(List<Long> docIDs) {
		...
		TypedQuery<Document> query = entitymanager.createQuery(
				"SELECT d from Document d WHERE d.iddocument IN :ids",
				Document.class);
	...
	public List<Bill> findBillsByID(List<Long> billIDs) {
		...
		TypedQuery<Bill> query = entitymanager.createQuery(
				"SELECT b from Bill b WHERE b.idbill IN :ids", Bill.class);
	...
	public List<Drawing> findDrawingsByID(List<Long> drawingIDs) {
		createEntityManager();
		TypedQuery<Drawing> query = entitymanager.createQuery(
				"SELECT d from Drawing d WHERE d.iddrawing IN :ids",
				Drawing.class);
	...
	public List<Project> findProjectsByID(List<Long> projectIDs) {
		...
		TypedQuery<Project> query = entitymanager.createQuery(
				"SELECT d from Project d WHERE d.idproject IN :ids",
				Project.class);
	...
	public List<Payment> findPaymentsByID(List<Long> paymentIDs) {
		...
		TypedQuery<Payment> query = entitymanager.createQuery(
				"SELECT d from Payment d WHERE d.idpayment IN :ids",
				Payment.class);
	...
	public int findLastSequence(Class className) {
		...
		TypedQuery<Object> query = entitymanager.createNamedQuery(
				className.getSimpleName() + ".findAllByReverseNumber",
				className);
	...
	public int findLastReceiptSequence() {
		createEntityManager();
		String sequence = null;
		int seqNum = -1;
		TypedQuery<Payment> query = entitymanager.createNamedQuery(
				"Payment.findAllByReverseReceiptNumber", Payment.class);
		entitymanager.setProperty("disableRetiredFeature", 1);
		query.setMaxResults(1);
		Object result = null;
		try {
		...
		if (result instanceof Payment) {
			sequence = ((Payment) result).getReceiptNumber();
		}
